---
layout: post
title: "Setting up a nice Git shell"
date: 2015-10-28 -0800
comments: true
...
At the same time, I often hear from people that they don't need a GUI because they're perfectly happy to use the command line. If you feel this way, that's great! I hope to change your mind in a set of upcoming blog posts that will show how GitHub Desktop augments and complements the command line in a nice and powerful way.

In the meanwhile, I think it's important to know that __GitHub Desktop is the easiest, fastest, and best way to get the Git command line set up on a Windows machine and keep it up to date.__
...
By default, the Git Shell icon launches PowerShell with Git set up. You can also launch the Git Shell from within the Desktop GUI application by using the `~` key or by using the gear menu in the top right and selecting "Open Shell."
...
I can't take credit for it. The first instance I've found [is on Twitter](https://twitter.com/aaronjorbin/status/576487768530001920) by Aaron Jorbin.
...
One thing to note is that if you PowerShell via some other means other than our Git Shell icon or the Desktop application, our version of Git won't be there. That's because we install a portable version of Git that does not change any of your system settings.
...
The other thing to note is if you have already customized PowerShell with a profile, launching the Git Shell via the GitHub Desktop application or Git Shell icon doesn't load your profile. We worried about conflicts with existing Posh-Git installs or whatever you might have. Instead, we execute a custom profile script if it exists, `GitHub.PowerShell_profile.ps1`.

Just create one in the same directory as your `$profile` script (typcially `%UserProfile%\Documents\WindowsPowershell\`).
...
* Under `Tabs`, paste the path `%SystemRoot%\syswow64\WindowsPowerShell\v1.0\powershell.exe` into the "Shell" text box. If you want to get really fancy, you could use one of the GitHub icons here such as http://haacked.com/images/icons/Shell.ico or http://haacked.com/images/icons/App.ico
...
![consolez-8bit](https://cloud.githubusercontent.com/assets/19977/10810919/7c4a373e-7dc3-11e5-89ea-76d9ebd88003.png)
